...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('forex_ml_server.log'),
...
class UnifiedTradingSystem:
    ...
    def load_existing_models(self):
        ...
        server_stats['models_loaded'] = len(self.models)
    ...
    def load_historical_data(self, symbol, timeframe, limit=50000):
        ...
        try:
            if not os.path.exists(self.historical_db):
                logger.warning("Historical database not found, using live data only")
                return None
    ...
    def train_model(self, symbol, timeframe):
        ...
        if df is None or len(df) < 1000:
            logger.warning(f"Not enough data for {symbol} {timeframe}, using simple model")
            return self.create_simple_model(symbol, timeframe)
        ...
        try:
            ...
            logger.info(f"   ✅ LightGBM: {lgb_model.score(X_test_scaled, y_test):.2%}")
        except Exception as e:
            logger.error(f"   ❌ LightGBM failed: {str(e)}")
        ...
        try:
            ...
            logger.info(f"   ✅ XGBoost: {xgb_model.score(X_test_scaled, y_test):.2%}")
        except Exception as e:
            logger.error(f"   ❌ XGBoost failed: {str(e)}")
            
        # 3. Random Forest (always works)
        ...
        logger.info(f"   ✅ Random Forest: {rf_model.score(X_test_scaled, y_test):.2%}")
    ...
    def create_simple_model(self, symbol, timeframe):
        """Create a simple model when no data available"""
        model_key = f"{symbol}_{timeframe}"
        self.models[model_key] = {
            'simple': True,
            'symbol': symbol,
    ...
    def predict(self, symbol, timeframe, features):
        ...
        if model_key not in self.models:
            return 2, 0.5  # Hold with low confidence
            
        model_data = self.models[model_key]
        
        # Check if simple model
        if model_data.get('simple'):
    ...
    def simple_prediction(self, features):
        """Simple prediction based on indicators"""
        try:
...
def retrain():
    ...
    try:
        ...
        unified_system.train_model(symbol, timeframe)
