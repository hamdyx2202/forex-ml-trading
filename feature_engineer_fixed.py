#!/usr/bin/env python3
"""
Fixed Feature Engineering Module
...
class FeatureEngineer:
    """Feature engineering for forex data - Fixed version"""
    ...
    def add_time_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """إضافة ميزات زمنية - Fixed version"""
        df = df.copy()
        
        # تأكد من أن لدينا datetime index أو عمود
        if 'datetime' in df.columns:
            time_col = pd.to_datetime(df['datetime'])
        elif df.index.name == 'datetime':
            time_col = pd.to_datetime(df.index)
        else:
            # إنشاء datetime من time إذا كان timestamp
    ...
    def add_market_structure(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [20, 50, 100]:
            ...
            df[f'trend_strength_{period}'] = abs(df['close'] - df[f'SMA_{period}']) / df[f'ATR_14']
        
        # Market regime
        df['volatility_regime'] = pd.qcut(df['ATR_14'], q=3, labels=['low', 'medium', 'high'])
        df['volume_regime'] = pd.qcut(df['volume'], q=3, labels=['low', 'medium', 'high'])
